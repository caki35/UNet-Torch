...
def test_single(model, device, input_size, image_list, output_save_dir):
    ...
    for img_path in tqdm(image_list):
        ...
        mask = cv2.resize(cv2.imread(mask_path, 0), input_size)*255
        ...
        seperater = np.zeros([img_org.shape[1], 15, 3], dtype=np.uint8)
        seperater.fill(155)
        ...
        rgb_mask_pred = create_rgb_mask(pred_bin_img, label_colors)
        rgb_mask_pred = cv2.cvtColor(rgb_mask_pred, cv2.COLOR_BGR2RGB)

        save_img_bin = np.hstack(
            [img8_8bit, seperater, rgb_mask, seperater, rgb_mask_pred])
        cv2.imwrite(os.path.join(results_save_dir_images,
