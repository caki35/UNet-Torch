...
def test_multiple_DM_reg(model, device, input_size, ch, Num_Class, image_list, save_dir):
    ...
    for img_path in tqdm(image_list):
        ...
        model_immune, model_other = model
        model_immune.to(device=device)
        model_immune.eval()
        model_other.to(device=device)
        model_other.eval()
        with torch.no_grad():
            output_immune = model_immune(img_input) 
            output_other = model_other(img_input)
...
def main():
    ...
    save_dir = 'doubledeneme2'
    test_path = '/home/ocaki13/projects/serous/Datav2/processed/datasetv2_768_reg/fold3/test/'
    image_list = get_image_list(test_path)
    modelType = 'TransUnet' #Unet
    input_size = (768,768)
    use_cuda = True
    model_path_immune = '/home/ocaki13/projects/serous/exps/datav2_exp3/doubleSC/fold3/seros_exp3_64TransUNet_singleImmune_fold3/seros_exp3_64TransUNet_singleImmune_fold3_seed35/epoch58.pt'
    model_path_other = '/home/ocaki13/projects/serous/exps/datav2_exp3/doubleSC/fold3/seros_exp3_64TransUNet_singleOther_fold3/seros_exp3_64TransUNet_singleOther_fold3_seed35/epoch56.pt'
    ...
    if modelType == 'TransUnet':
        ...
        model_immune = ViT_seg(config_vit, img_size=input_size[0], num_classes=Num_Class).cuda()
        model_other = ViT_seg(config_vit, img_size=input_size[0], num_classes=Num_Class).cuda()
    ...
    model_immune.load_state_dict(torch.load(model_path_immune))
    model_other.load_state_dict(torch.load(model_path_other))
    model = [model_immune, model_other]
    
    resDict = test_multiple_DM_reg(model, device, input_size, ch, Num_Class, image_list, save_dir)
